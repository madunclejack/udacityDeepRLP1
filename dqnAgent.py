...
#from networkModel import QNetwork
...
class Agent():
    ...
    def __init__(self, stateSize, actionSize, seed, shouldUseDDQN=False, fileName=None):
        ...
        # Decide if we should use Double DQN
        self.ddqn = shouldUseDDQN
        
        # Varaible to keep track of steps in between each NN learning Update
        self.stepNum = 0
        
        # Variable to control learn every
        self.learnEveryCounter = 0
        self.learnEvery = 5
        ...
        self.QNet_Target = agentQNetwork(self.stateSize, self.actionSize, seed=self.seed).to(device)
        self.QNet_Local = agentQNetwork(self.stateSize, self.actionSize, seed=self.seed).to(device)
        
        # Used to run the network for inference
        if fileName:
            weights = torch.load(fileName)
            self.QNet_Local.load_state_dict(weights)
            self.QNet_Target.load_state_dict(weights)
        
        # Set up the Optimizer for training the networks.
        # Can set learning rate (i.e. gradient step size) and apply momentum if desired
    ...
    def act(self, state, epsilon):
        ...
        if random.random() > epsilon:
            ...
            # in PyTorch. The new way is to use .detach. 
            # A Variable is a Tensor object with history tracking for autograd method.
            # .numpy returns as Numpy ndarray
            return np.argmax(actionVals.cpu().data.numpy())
        else:
    ...
    def learn(self, experiences):
        ...
        # Get network output for the next state. Note that nn.Module class inherently calls the 
        # forward method
        # .max acts on the Tensor, arguemnt is the dim. Returns (value, indices) where 
        # value is the max val in each row and indices are the location of each max val
        # in the row. With the dim argument, max will peform torch.squeeze along that dim.
        # .squeeze removes all dims with size = 1, or just the specified dim.
        #  that is, with dim = 1, (Ax1xB) -> (AxB)
        #  .unsqueeze will reformat to dimensions with 1 at the specified dim (0 - row, 1 - column)
        
        if self.ddqn:
            next_actions = self.QNet_Local(next_states).max(1)[1].detach().unsqueeze(1)
            Q_Targets_Next = self.QNet_Target(next_states).gather(1, next_actions).detach()
        else:
        ...
        # .gather(dim, index) selects values from each row (or column) of a Tensor. 
        # Row/Column specified by dim.
    ...
    def softUpdate(self):
        ...
        """
        for targetParams, localParams in zip(self.QNet_Target.parameters(),\
...
class ReplayBuffer():
    ...
    def memSample(self):
        ...
        states = torch.from_numpy(np.vstack([mem.state for mem in memorySample \
                                             if mem is not None])).float().to(device)
        actions = torch.from_numpy(np.vstack([mem.action for mem in memorySample\
                                              if mem is not None])).long().to(device)
        rewards = torch.from_numpy(np.vstack([mem.reward for mem in memorySample\
                                              if mem is not None])).float().to(device)
        nextStates = torch.from_numpy(np.vstack([mem.nextState for mem in memorySample\
                                                 if mem is not None])).float().to(device)
        dones = torch.from_numpy(np.vstack([mem.done for mem in memorySample\
                                            if mem is not None]).astype(np.uint8)).float().to(device)
